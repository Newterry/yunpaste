...
![Version](https://img.shields.io/badge/version-1.15.2-ff655f)
![Node.js](https://img.shields.io/badge/Node.js-24-339933?logo=nodedotjs&logoColor=white)
![React](https://img.shields.io/badge/React-19-149eca?logo=react&logoColor=white)
![Docker](https://img.shields.io/badge/Docker-ready-2496ed?logo=docker&logoColor=white)
![Docker Pulls](https://img.shields.io/docker/pulls/newterry/yunpaste?logo=docker&label=Docker%20Hub)
...
<p align="center">
  <img src="public/assets/yunpaste-social-preview.webp" alt="云粘贴让一处复制的文件在手机、平板和电脑之间随处使用" width="920">
</p>

云粘贴是一款轻量、高效的自托管跨设备文件助手。文件留在自己的云端，手机无需安装
额外 App；不主动下载，就不占手机文件空间。在一台设备粘贴文字或上传文件，其他手机、
平板和电脑打开浏览器即可继续使用。

相比把文件堆进聊天记录，云粘贴提供真正的文件夹、搜索、排序、收藏、回收站和多格式
在线预览。需要发送给别人时，只需生成一条安全的限时链接，对方无需注册或安装应用。

> 微信适合聊天，云粘贴专注于文件流转。

| 使用体验 | 聊天式文件传输 | 云粘贴 |
| --- | --- | --- |
| 手机负担 | 依赖完整聊天应用和本地缓存 | 浏览器即开即用，不下载不占手机文件空间 |
| 查找文件 | 文件混在消息记录中 | 文件夹、模糊搜索、排序和多种视图 |
| 跨设备使用 | 依赖账号和专用客户端 | 任意设备打开网页即可使用 |
| 分享给别人 | 通常需要联系人或群聊 | 一条限时链接分享给任何人 |
| 查看内容 | 以消息附件为主 | 图片、PDF、Office、音视频和代码集中预览 |
...
无需额外安装手机 App，也不必把所有文件下载到手机。云粘贴在服务器上完成保存、整理、
搜索和预览；只有主动下载的文件才会写入手机文件空间。浏览器打开即用，退出后不会留下
一套庞大的聊天记录和附件缓存。

### 2. 一处复制，所有设备继续使用

电脑粘贴的文字、图片和文件，会立即出现在同一账号的手机、平板和其他电脑上。无需反复
发送给自己，也不用在不同客户端之间寻找“文件传输助手”。

### 3. 一条链接，快捷分享给任何人

选择文件即可生成高熵限时链接。接收方无需成为联系人、无需注册，也无需安装云粘贴，
打开浏览器即可安全预览或下载。链接最长有效 7 天，可随时撤销。

### 4. 隐私边界清晰，不把管理员变成“万能文件查看者”

每个用户的文件、文件夹、搜索结果、收藏、回收站和个人 WebDAV 都按用户隔离。
管理员负责用户、角色、配额和系统策略，但不能浏览其他用户的私有文件。文件分享
通过独立的高熵令牌访问，不能根据一个分享地址推导同一用户的其他文件。

### 5. 不只是“粘贴板”，而是一套完整文件管理器

“我的文件”支持文件夹、面包屑、列表/网格/图片视图、排序、分页、重命名、复制、
剪切、粘贴、移动到、复制到、收藏、回收站、批量选择和右键菜单。文本与文件使用
同一个“添加内容”入口，也支持拖拽和多文件上传，操作方式接近桌面文件管理器。

### 6. 强大的本地预览，私有文件不必交给第三方在线转换
...
生产镜像以 Node.js 26 运行，内置固定版本的 rclone 1.74.4，并同时支持
...
```bash
docker pull newterry/yunpaste:1.15.2
```
...
| 变量 | 默认值 | 作用 |
| --- | --- | --- |
| `TIEYUN_IMAGE_TAG` | `1.15.2` | 构建标签和运行版本 |
...
推送形如 `v1.15.1` 的版本标签后，`.github/workflows/release-flutter.yml` 会自动构建 Flutter 各平台产物并创建 GitHub Release；Android 正式上架前应配置固定签名 Secrets，未配置时 workflow 会使用临时 CI key 生成仅供安装验收的 APK/AAB，详见 [`flutter_app/README.md`](flutter_app/README.md) 的“GitHub Actions 自动构建与 Release”章节。
