...
function WebdavRow({ item, selected, onSelect, onOpen, onDownload, onRename, onCopy, onMove, onClipboardCopy, onClipboardMove, onDelete }: {
  ...
  return <div ref={menuRoot as React.RefObject<HTMLDivElement>} className={`webdav-file-row ${selected ? "is-selected" : ""} ${menu ? "has-open-menu" : ""}`} onDoubleClick={onOpen} onContextMenu={(event) => { event.preventDefault(); setMenuPoint({ x: event.clientX, y: event.clientY }); setMenu(true); }}><span className="webdav-file-name"><button className={`fake-checkbox ${selected ? "is-checked" : ""}`} onClick={onSelect}>{selected && <Check />}</button><button onClick={onOpen}>{item.isDir ? <Folder /> : <File />}<span><strong>{item.name}</strong><small>{item.isDir ? "文件夹" : item.mime || "文件"}</small></span></button></span><span>{item.isDir ? "—" : formatBytes(item.size)}</span><span>{item.modifiedAt ? formatDate(item.modifiedAt) : "—"}</span><span className="webdav-row-actions"><button className="icon-button" onClick={() => { setMenuPoint(undefined); setMenu((value) => !value); }}><MoreHorizontal /></button>{menu && <ContextMenuSurface point={menuPoint}><button onClick={() => { onOpen(); setMenu(false); }}><FolderOpen />{item.isDir ? "打开" : "预览"}</button>{!item.isDir && <button onClick={() => { onDownload(); setMenu(false); }}><Download />下载</button>}<button onClick={() => { onRename(); setMenu(false); }}><Pencil />重命名</button><button onClick={() => { onCopy(); setMenu(false); }}><Copy />复制到…</button><button onClick={() => { onMove(); setMenu(false); }}><Move />移动到…</button><button onClick={() => { onClipboardCopy(); setMenu(false); }}><Clipboard />复制</button><button onClick={() => { onClipboardMove(); setMenu(false); }}><Move />剪切</button><button onClick={() => { onDelete(); setMenu(false); }}><Trash2 />删除</button></ContextMenuSurface>}</span></div>;
}
