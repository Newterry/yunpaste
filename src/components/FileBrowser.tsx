...
import { ContextMenuSurface, type ContextMenuPoint, useDismissibleMenu } from "./ContextMenuSurface";
import type {
...
function FileMenu({ file, view, onPreview, onOpen, onPatch, onDelete, onDownload, onCopyLink, onClipboard, onChooseDestination, onSendWebdav, close, point }: FileActions & { close: () => void; point?: ContextMenuPoint }) {
  const act = (fn: () => void) => { fn(); close(); };
  return <ContextMenuSurface point={point}><button onClick={() => act(() => onOpen(file))}><FolderOpen />新窗口打开</button><button onClick={() => act(() => onPreview(file))}><CirclePlay />在侧栏预览</button>{view === "trash" ? <><button onClick={() => act(() => onPatch(file, { is_trashed: 0 }))}><RotateCcw />恢复</button><button onClick={() => act(() => onDelete(file))}><Trash2 />永久删除</button></> : <><button onClick={() => act(() => onCopyLink(file, 1))}><Link2 />快速分享（1 天）</button><button onClick={() => act(() => { const name = window.prompt("输入新的文件名", file.name)?.trim(); if (name && name !== file.name) onPatch(file, { name }); })}><Pencil />重命名</button><button onClick={() => act(() => onChooseDestination("copy", file))}><Copy />复制到…</button><button onClick={() => act(() => onChooseDestination("move", file))}><Move />移动到…</button><button onClick={() => act(() => onClipboard("copy", file))}><Clipboard />复制</button><button onClick={() => act(() => onClipboard("move", file))}><Move />剪切</button><button onClick={() => act(() => onSendWebdav(file))}><UploadCloud />发送到 WebDAV</button><button onClick={() => act(() => onPatch(file, { is_favorite: file.is_favorite ? 0 : 1 }))}><Heart />{file.is_favorite ? "取消收藏" : "收藏"}</button><button onClick={() => act(() => onDownload(file))}><Download />下载</button><button onClick={() => act(() => onPatch(file, { is_trashed: 1 }))}><Trash2 />移到回收站</button></>}</ContextMenuSurface>;
}
...
const FileRow = memo(function FileRow(props: FileActions & { selected: boolean; active: boolean; onSelect: (file: FileItem, additive?: boolean) => void }) {
  ...
  return <div ref={menuRoot as React.RefObject<HTMLDivElement>} className={`manager-row ${selected ? "is-selected" : ""} ${active ? "is-active" : ""}`} role="row" onDoubleClick={() => onOpen(file)} onContextMenu={(event) => { event.preventDefault(); onSelect(file); setMenuPoint({ x: event.clientX, y: event.clientY }); setMenu(true); }}><span className="manager-name"><button className={`fake-checkbox ${selected ? "is-checked" : ""}`} onClick={() => onSelect(file, true)}>{selected && <Check />}</button><button className="file-name" onClick={() => { onSelect(file); onPreview(file); }}><FileTypeIcon file={file} /><i><strong>{file.name}</strong><small>{kindMeta[file.kind]?.label || "文件"}</small></i></button></span><span>{formatBytes(file.size)}</span><span>{formatDate(file.updated_at)}</span><span>{file.is_shared ? <SharedStatus file={file} /> : file.is_favorite ? <i className="manager-status"><Star />收藏</i> : <i className="manager-status">私有</i>}</span><span className="manager-more"><button className="icon-button" onClick={() => { setMenuPoint(undefined); setMenu((value) => !value); }} aria-expanded={menu} aria-label={`打开 ${file.name} 的文件菜单`}><MoreHorizontal /></button>{menu && <FileMenu {...props} point={menuPoint} close={() => setMenu(false)} />}</span></div>;
});
...
function FolderMenu({ folder, view, onOpen, onPatch, onDelete, onClipboard, onChooseDestination, close, point }: FolderActions & { close: () => void; point?: ContextMenuPoint }) {
  const act = (fn: () => void) => { fn(); close(); };
  return <ContextMenuSurface point={point}><button onClick={() => act(() => onOpen(folder.id))}><FolderOpen />打开文件夹</button>{view === "trash" ? <><button onClick={() => act(() => onPatch(folder, { is_trashed: 0 }))}><RotateCcw />恢复</button><button onClick={() => act(() => onDelete(folder))}><Trash2 />永久删除</button></> : <><button onClick={() => act(() => { const name = window.prompt("输入新的文件夹名称", folder.name)?.trim(); if (name && name !== folder.name) onPatch(folder, { name }); })}><Pencil />重命名</button><button onClick={() => act(() => onChooseDestination("copy", undefined, folder))}><Copy />复制到…</button><button onClick={() => act(() => onChooseDestination("move", undefined, folder))}><Move />移动到…</button><button onClick={() => act(() => onClipboard("copy", undefined, folder))}><Clipboard />复制</button><button onClick={() => act(() => onClipboard("move", undefined, folder))}><Move />剪切</button><button onClick={() => act(() => onPatch(folder, { is_favorite: folder.is_favorite ? 0 : 1 }))}><FolderHeart />{folder.is_favorite ? "取消收藏" : "收藏"}</button><button onClick={() => act(() => onPatch(folder, { is_trashed: 1 }))}><Trash2 />移到回收站</button></>}</ContextMenuSurface>;
}
...
const FolderRow = memo(function FolderRow(props: FolderActions & { selected: boolean; onSelect: (folder: FolderItem, additive?: boolean) => void }) {
  ...
  const [menuPoint, setMenuPoint] = useState<ContextMenuPoint>();
  const menuRoot = useDismissibleMenu(menu, () => setMenu(false));
  return <div ref={menuRoot as React.RefObject<HTMLDivElement>} className={`manager-row manager-row--folder ${selected ? "is-selected" : ""}`} role="row" onDoubleClick={() => onOpen(folder.id)} onContextMenu={(event) => { event.preventDefault(); onSelect(folder); setMenuPoint({ x: event.clientX, y: event.clientY }); setMenu(true); }}><span className="manager-name"><button className={`fake-checkbox ${selected ? "is-checked" : ""}`} onClick={() => onSelect(folder, true)}>{selected && <Check />}</button><button className="file-name" onClick={() => onSelect(folder)}><span className="folder-icon"><Folder /></span><i><strong>{folder.name}</strong><small>文件夹 · 双击打开</small></i></button></span><span>—</span><span>{formatDate(folder.updated_at)}</span><span>{folder.is_favorite ? <i className="manager-status"><Star />永久保留</i> : <i className="manager-status">文件夹</i>}</span><span className="manager-more"><button className="icon-button" onClick={() => { setMenuPoint(undefined); setMenu((value) => !value); }}><MoreHorizontal /></button>{menu && <FolderMenu {...props} point={menuPoint} close={() => setMenu(false)} />}</span></div>;
});
...
const FolderCard = memo(function FolderCard(props: FolderActions & { selected: boolean; onSelect: (folder: FolderItem, additive?: boolean) => void }) {
  ...
  const [menuPoint, setMenuPoint] = useState<ContextMenuPoint>();
  const menuRoot = useDismissibleMenu(menu, () => setMenu(false));
  return <article ref={menuRoot as React.RefObject<HTMLElement>} className={`file-card manager-card manager-folder-card ${selected ? "is-selected" : ""}`} onDoubleClick={() => onOpen(folder.id)} onContextMenu={(event) => { event.preventDefault(); onSelect(folder); setMenuPoint({ x: event.clientX, y: event.clientY }); setMenu(true); }}><button className={`fake-checkbox ${selected ? "is-checked" : ""}`} onClick={() => onSelect(folder, true)}>{selected && <Check />}</button><button className="file-card__favorite" onClick={() => onPatch(folder, { is_favorite: folder.is_favorite ? 0 : 1 })}><Heart fill={folder.is_favorite ? "currentColor" : "none"} /></button><button className="file-card__preview folder-card__preview" onClick={() => onOpen(folder.id)}><Folder /></button><div className="file-card__info"><strong>{folder.name}</strong><span>文件夹 · {formatDate(folder.updated_at, false)}</span></div><button className="manager-card__menu icon-button" onClick={() => { setMenuPoint(undefined); setMenu((value) => !value); }}><MoreHorizontal /></button>{menu && <FolderMenu {...props} point={menuPoint} close={() => setMenu(false)} />}</article>;
});
